...
class UserControllerE2ETest {
	...
	@Test
	@DisplayName("Set profile image with WebP should fail")
	void setProfileImageWebPSystemTest() {
		// GIVEN: upload a WebP image
		byte[] webpData = new byte[] { 1, 2, 3, 4, 5 };

		Response uploadResponse = given()
				.cookie("AuthToken", authToken)
				.multiPart("files", "avatar.webp", webpData, "image/webp")
				.when()
				.post("http://localhost:" + port + "/api/v1/images")
				.then()
				.statusCode(200)
	...
	@Test
	@DisplayName("Get user liked comments should succeed")
	void getLikedCommentsSystemTest() {
		// WHEN & THEN
		given()
				.cookie("AuthToken", authToken)
				.queryParam("page", 0)
	...
	@Test
	@DisplayName("Get other user liked comments should fail")
	void getLikedCommentsAccessDeniedSystemTest() {
		// WHEN & THEN
		given()
				.cookie("AuthToken", authToken)
				.queryParam("page", 0)
				.queryParam("size", 10)
				.when()
				.get("/1/liked-comments")
				.then()
				.statusCode(403);
	}

	@Test
	@DisplayName("Get user comments with authentication should succeed")
	void getUserCommentsAuthenticatedSystemTest() {
		// WHEN & THEN
		given()
				.cookie("AuthToken", authToken)
				.queryParam("page", 0)
	...
	@Test
	@DisplayName("Get user comments without authentication should succeed")
	void getUserCommentsAnonymousSystemTest() {
		// WHEN & THEN
		given()
				.queryParam("page", 0)
				.queryParam("size", 10)
